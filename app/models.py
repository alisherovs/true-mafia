...
class CoinFlipGame(Base):
    ...
    choice: Mapped[str] = mapped_column(String(16))  # heads | tails
    result: Mapped[str] = mapped_column(String(16))  # heads | tails
    won: Mapped[bool] = mapped_column(Boolean, default=False)
    payout: Mapped[int] = mapped_column(Integer, default=0)
...
class Clan(Base):
    __tablename__ = "clans"
    __table_args__ = (
        Index("ix_clans_name", "name"),
        Index("ix_clans_owner_id", "owner_id"),
        Index("ix_clans_rating", "rating"),
        Index("ix_clans_xp", "xp"),
        Index("ix_clans_status", "status"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    name: Mapped[str] = mapped_column(String(64), unique=True, index=True)
    bio: Mapped[str] = mapped_column(Text, default="We fight together!")
...
class ClanMember(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_clan_members_user_tg", "user_telegram_id"),
    ...
    joined_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    personal_wins: Mapped[int] = mapped_column(Integer, default=0)
    personal_losses: Mapped[int] = mapped_column(Integer, default=0)
    personal_games: Mapped[int] = mapped_column(Integer, default=0)
...
class ClanApplication(Base):
    ...
    __table_args__ = (
        Index("ix_clan_apps_clan_status", "clan_id", "status"),
        Index("ix_clan_apps_user_status", "user_telegram_id", "status"),
    )
    ...
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"), index=True)
    user_telegram_id: Mapped[int] = mapped_column(BigInteger, index=True)
    status: Mapped[str] = mapped_column(String(32), default="pending")  # pending | accepted | rejected | cancelled
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    processed_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    processed_by: Mapped[Optional[int]] = mapped_column(BigInteger, nullable=True)


class ClanTransaction(Base):
    __tablename__ = "clan_transactions"
    __table_args__ = (
        Index("ix_clan_tx_clan_id", "clan_id"),
        Index("ix_clan_tx_user_tg", "user_telegram_id"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    clan_id: Mapped[int] = mapped_column(ForeignKey("clans.id", ondelete="CASCADE"), index=True)
    user_telegram_id: Mapped[int] = mapped_column(BigInteger)
    type: Mapped[str] = mapped_column(String(64))  # create_clan | name_change | bio_change
    currency: Mapped[str] = mapped_column(String(32))  # diamonds | dollar
    amount: Mapped[int] = mapped_column(Integer)
    description: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())


class ClanGameReward(Base):
    """Idempotency tracking for game rewards awarded to clans."""
    __tablename__ = "clan_game_rewards"
    __table_args__ = (
        UniqueConstraint("game_id", "player_telegram_id", name="uq_clan_game_player_reward"),
        Index("ix_clan_rewards_clan_id", "clan_id"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    game_id: Mapped[int] = mapped_column(Integer, index=True)
    clan_id: Mapped[int] = mapped_column(ForeignKey("clans.id", ondelete="CASCADE"), index=True)
    player_telegram_id: Mapped[int] = mapped_column(BigInteger, index=True)
    result: Mapped[str] = mapped_column(String(16))  # win | loss
...
class ClanAuditLog(Base):
    ...
    __table_args__ = (
        Index("ix_clan_audit_clan_id", "clan_id"),
    )
