...
async def _ensure_lightweight_columns(conn) -> None:
    ...
    await conn.execute(
        text("CREATE INDEX IF NOT EXISTS ix_coin_flip_user_created ON coin_flip_games(user_telegram_id, created_at)")
    )
    await conn.execute(
        text("CREATE INDEX IF NOT EXISTS ix_coin_flip_user_won ON coin_flip_games(user_telegram_id, won)")
    )
    await conn.execute(
        text("CREATE INDEX IF NOT EXISTS ix_joker_user_created ON joker_cards_games(user_telegram_id, created_at)")
    )
    await conn.execute(
        text("CREATE INDEX IF NOT EXISTS ix_joker_user_won ON joker_cards_games(user_telegram_id, won)")
    )
    await conn.execute(
        text("CREATE INDEX IF NOT EXISTS ix_joker_diff_created ON joker_cards_games(difficulty, created_at)")
    )
    ...
    await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_clans_name ON clans(name)"))
    await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_clans_owner_id ON clans(owner_id)"))
    await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_clans_rating ON clans(rating)"))
    await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_clans_xp ON clans(xp)"))
    await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_clans_status ON clans(status)"))
    await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_clan_members_user_tg ON clan_members(user_telegram_id)"))
    await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_clan_members_clan_role ON clan_members(clan_id, role)"))
    await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_clan_apps_clan_status ON clan_applications(clan_id, status)"))
    await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_clan_apps_user_status ON clan_applications(user_telegram_id, status)"))
    await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_clan_rewards_clan_id ON clan_game_rewards(clan_id)"))
